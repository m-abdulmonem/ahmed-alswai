<?php

$settings = settings();
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="A fully featured admin theme which can be used to build CRM, CMS, etc.">
    <meta name="author" content="Coderthemes">

    <link rel="shortcut icon" href="/Site/images/favicon_1.ico">

    <title><?php
        if (isset($Name) && !empty($Name)){
            echo $Name . " | ";
        }
        echo settings()['title'];
        ?></title>

    <!-- Pulgins -->
    <?php
//    if (isset($css) && !empty($css)){
//        foreach ($css as $style){
//            echo "<link href='/Admin/css/plugins/" . $css . ".css' rel='stylesheet' />";
//        }
//    }
    ?>
    <link href="/Admin/css/bootstrap-rtl.min.css" rel="stylesheet" type="text/css" />
    <link href="/Admin/css/core.css" rel="stylesheet" type="text/css" />
    <link href="/Admin/css/components.css" rel="stylesheet" type="text/css" />
    <link href="/Admin/css/pages.css" rel="stylesheet" type="text/css" />
    <link href="/Admin/css/responsive.css" rel="stylesheet" type="text/css" />
    <link href="/Site/css/font-awesome.min.css" rel="stylesheet" type="text/css" />
    <!-- Plugins -->
    <link href="/Admin/css/plugins/froala_editor.css" rel="stylesheet" type="text/css" />
    <link href="/Admin/css/plugins/froala_style.css" rel="stylesheet" type="text/css" />
    <link href="/Admin/css/plugins/ladda.css" rel="stylesheet" type="text/css">
    <?php
    // echo "<link href='/Admin/css/plugins/" . $css . ".css' rel='stylesheet' />";
    ?>
    <link href="/Admin/css/Style.css" rel="stylesheet" type="text/css" />

    <!-- HTML5 Shiv and Respond.js IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
    <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
    <script src="https://oss.maxcdn.com/libs/respond.js/1.3.0/respond.min.js"></script>
    <![endif]-->

</head>

<body class="fixed-left">

<!-- Begin page -->
<div id="wrapper">

